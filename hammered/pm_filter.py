...
async def advantage_spoll_choker(bot, query):
    ...
    if files:
        ...
        reqstr1 = callback_user_id(query) if query.from_user else 0
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        await query.message.edit_text(text=script.CLONE_START_TXT.format(callback_mention(query), me.username, me.first_name), reply_markup=reply_markup)
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    FRESH[key] = search
    temp.GETALL[key] = files
    req_user_id = message_user_id(message) if message.from_user else 0
    temp.SHORT[req_user_id] = message.chat.id
    btn = [
        [
            InlineKeyboardButton(
                text=f"[{get_size(filevj['file_size'])}] {format_file_name(filevj['file_name'], prefix=False)}", callback_data=f"file#{filevj['file_id']}"
    ...
    if imdb:
        try:
            cap = TEMPLATE.format(
                query=search,
                message_mention=message_mention(message) if message.from_user else "Anonymous",
        ...
        if message.from_user:
            temp.IMDB_CAP[message_user_id(message)] = cap
    else:
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if vj_search == True:
        vj_search_new = False
        vj_ai_msg = await reply_msg.edit_text("<b><i>Advance Ai Of Poppy Seeds Try To Find Your Movie With Your Wrong Spelling.</i></b>")
        movienamelist = []
        ...
        for movie in movienamelist:
            ...
            if mv_rqst.startswith(movie[0]):
                await auto_filter(client, movie, msg, reply_msg, vj_search_new)
                break
        reqst_gle = mv_rqst.replace(" ", "+")
        button = [[
            InlineKeyboardButton("Gᴏᴏɢʟᴇ", url=f"https://www.google.com/search?q={reqst_gle}")
