...
def safe_literal_buttons(btn):
    try:
        parsed = ast.literal_eval(btn or "[]")
        return parsed if isinstance(parsed, list) else []
    except (ValueError, SyntaxError):
        logger.warning("Invalid stored button markup")
        return []


def callback_user_id(query):
    return query.from_user.id if query.from_user else 0


def callback_first_name(query):
    return query.from_user.first_name if query.from_user else "User"


def callback_mention(query):
    return query.from_user.mention if query.from_user else "User"


def reply_user_id(query):
    reply = query.message.reply_to_message if query.message else None
    user = reply.from_user if reply else None
    return user.id if user else 0


def reply_message_id(query):
    reply = query.message.reply_to_message if query.message else None
    return reply.id if reply else 0
...
def message_mention(message):
    return message.from_user.mention if message.from_user else "Anonymous"
...
async def give_filter(client, message):
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        user_id = message_user_id(message) if message.from_user else None
        if settings['fsub'] is not None and message.from_user:
            try:
                btn = await pub_is_subscribed(client, message, settings['fsub'])
                if btn:
                    btn.append([InlineKeyboardButton("Unmute Me 🔕", callback_data=f"unmuteme#{int(user_id)}")])
                    await client.restrict_chat_member(chatid, message_user_id(message), ChatPermissions(can_send_messages=False))
                    await message.reply_photo(photo=random.choice(PICS), caption=f"👋 Hello {message_mention(message)},\n\nPlease join the channel then click on unmute me button. 😇", reply_markup=InlineKeyboardMarkup(btn), parse_mode=enums.ParseMode.HTML)
                    return
        ...
        if manual == False:
            ...
            try:
                ...
                grpid = await active_connection(str(message_user_id(message)))
        ...
        if total_results == 0:
            ...
            requester = message_mention(message) if message.from_user else "Anonymous"
            return await message.reply_text(f"<b>Hᴇʏ {requester}, {str(total_results)} ʀᴇsᴜʟᴛs ᴀʀᴇ ғᴏᴜɴᴅ ɪɴ ᴍʏ ᴅᴀᴛᴀʙᴀsᴇ ғᴏʀ ʏᴏᴜʀ ᴏ̨ᴜᴇʀʏ {search}. \n\nTʜɪs ɪs ᴀ sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ sᴏ ᴛʜᴀᴛ ʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ғɪʟᴇs ғʀᴏᴍ ʜᴇʀᴇ...\n\nJᴏɪɴ ᴀɴᴅ Sᴇᴀʀᴄʜ Hᴇʀᴇ - {GRP_LNK}</b>")
...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ai_search = True
        reply_msg = await bot.send_message(message_user_id(message), f"<b><i>Searching For {content} 🔍</i></b>", reply_to_message_id=message.id)
...
async def next_page(bot, query):
    ...
    if int(req) not in [callback_user_id(query), 0]:
        return await query.answer(script.ALRT_TXT.format(callback_first_name(query)), show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
   ...
   # if not search:
      #  await query.answer(script.OLD_ALRT_TXT.format(callback_first_name(query)),show_alert=True)
       # return
    ...
    temp.SHORT[callback_user_id(query)] = query.message.chat.id
...
async def advantage_spoll_choker(bot, query):
    ...
    movies = SPELL_CHECK.get(reply_message_id(query))
    if not movies:
        return await query.answer(script.OLD_ALRT_TXT.format(callback_first_name(query)), show_alert=True)
    user_id = int(user) if user and user.isdigit() else 0
    if user_id != 0 and callback_user_id(query) != user_id:
        return await query.answer(script.ALRT_TXT.format(callback_first_name(query)), show_alert=True)
    ...
    if gl == False:
        ...
        if k == False:
            ...
            if files:
                ...
                reqstr1 = callback_user_id(query) if query.from_user else None
                reqstr = await bot.get_users(reqstr1) if reqstr1 else None
                reqstr_id = reqstr.id if reqstr else "Anonymous"
                reqstr_mention = reqstr.mention if reqstr else "Anonymous"
                if NO_RESULTS_MSG:
                    await bot.send_message(chat_id=LOG_CHANNEL, text=(script.NORSLTS.format(reqstr_id, reqstr_mention, movie)))
...
async def years_cb_handler(client: Client, query: CallbackQuery):
    ...
    try:
        if int(callback_user_id(query)) not in [reply_user_id(query), 0]:
            return await query.answer(
                f"⚠️ ʜᴇʟʟᴏ{callback_first_name(query)},\nᴛʜɪꜱ ɪꜱ ɴᴏᴛ ʏᴏᴜʀ ᴍᴏᴠɪᴇ ʀᴇQᴜᴇꜱᴛ,\nʀᴇQᴜᴇꜱᴛ ʏᴏᴜʀ'ꜱ...",
                show_alert=True,
            )
    except:
        pass
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    try:
        if int(req) not in [reply_user_id(query), 0]:
            return await query.answer(
                f"⚠️ ʜᴇʟʟᴏ{callback_first_name(query)},\nᴛʜɪꜱ ɪꜱ ɴᴏᴛ ʏᴏᴜʀ ᴍᴏᴠɪᴇ ʀᴇQᴜᴇꜱᴛ,\nʀᴇQᴜᴇꜱᴛ ʏᴏᴜʀ'ꜱ...",
                show_alert=True,
            )
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        user_id = callback_user_id(query)
    ...
    if query.data.startswith("file"):
        clicked = callback_user_id(query)
        try:
            typed = reply_user_id(query)
        except:
            typed = callback_user_id(query)
        ...
        if CUSTOM_FILE_CAPTION:
            try:
                f_caption = CUSTOM_FILE_CAPTION.format(file_name='' if title is None else format_file_name(title),
        ...
        try:
            if settings['is_shortlink'] and not await db.has_premium_access(callback_user_id(query)):
                if clicked == typed:
                    temp.SHORT[clicked] = query.message.chat.id
                    await query.answer(url=f"https://telegram.me/{temp.U_NAME}?start=short_{file_id}")
                    return
                else:
                    await query.answer(f"Hᴇʏ {callback_first_name(query)}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇsᴛ. Rᴇǫᴜᴇsᴛ Yᴏᴜʀ's !", show_alert=True)
            elif settings['is_shortlink'] and await db.has_premium_access(callback_user_id(query)):
                if clicked == typed:
     ...
     #   if not search:
      ...
      #      return
        ...
        await send_all(client, callback_user_id(query), files, ident, query.message.chat.id, callback_first_name(query), query)
        ...
        files, n_offset, total = await get_search_results(query.message.chat.id, search, offset=int(offset), filter=True)
        await send_all(client, callback_user_id(query), files, ident, query.message.chat.id, callback_first_name(query), query)
        await query.answer(f"Hey {callback_first_name(query)}, All files on this page has been sent successfully to your PM !", show_alert=True)
     ...
     #   if not search:
      ...
      #      return
        ...
        userid = callback_user_id(query) if query.from_user else None
        st = await client.get_chat_member(grp_id, userid)
        if (
        ...
        if callback_user_id(query) in ADMINS:
        ...
        btn2 = [[
            ...
            InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/letswatchitnow")
        ]]
        if callback_user_id(query) in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        ident, from_user = query.data.split("#")
        if int(callback_user_id(query)) == int(from_user):
            user = await client.get_users(from_user)
        ...
        await query.message.edit_text(
            text=script.START_TXT.format(callback_mention(query), temp.U_NAME, temp.B_NAME),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        ...
        await query.message.edit_text(
            text=script.ALL_FILTERS.format(callback_mention(query)),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        ...
        await query.message.edit_text(
            text=script.HELP_TXT.format(callback_mention(query)),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Sᴏᴜʀᴄᴇ Cᴏᴅᴇ', url="https://t.me/letswatchitnow")
        ],[
        ...
        await query.message.edit_text(
            text=script.SUBSCRIPTION_TXT.format(REFERAL_PREMEIUM_TIME, temp.U_NAME, callback_user_id(query), REFERAL_COUNT),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        ...
        btn = [[
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
            InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="t.me/letswatchitnow")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        grpid = await active_connection(str(callback_user_id(query)))
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if not spoll:
        ...
        if not message:
            return await msg.answer(script.OLD_ALRT_TXT.format(callback_first_name(msg)), show_alert=True)
        sender = message.from_user or message.sender_chat
        if not sender:
            return
    ...
    req = message_user_id(message) if message.from_user else 0
    FRESH[key] = search
    temp.GETALL[key] = files
    temp.SHORT[req] = message.chat.id  # ✅ safe
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
    ...
    if imdb:
        ...
        if sender:
            temp.IMDB_CAP[sender.id] = cap
        if not settings["button"]:
            cap+="<b>\n\n<u>🍿 Your Movie Files 👇</u></b>\n"
            for file in files:
                cap += f"<b>\n📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file['file_id']}'>[{get_size(file['file_size'])}] {format_file_name(file['file_name'])}\n</a></b>"
    else:
        if settings["button"]:
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            try:
                if settings['auto_delete']:
                    asyncio.create_task(delete_messages_later(300, hehe, message))
            except KeyError:
            ...
            try:
               if settings['auto_delete']:
                    asyncio.create_task(delete_messages_later(300, hmm, message))
            except KeyError:
                await save_group_settings(message.chat.id, 'auto_delete', True)
                asyncio.create_task(delete_messages_later(300, hmm, message))
        except Exception as e:
            ...
            try:
                if settings['auto_delete']:
                    asyncio.create_task(delete_messages_later(300, fek, message))
            except KeyError:
                await save_group_settings(message.chat.id, 'auto_delete', True)
                asyncio.create_task(delete_messages_later(300, fek, message))
    else:
        ...
        try:
            if settings['auto_delete']:
                asyncio.create_task(delete_messages_later(300, fuk, message))
        except KeyError:
            await save_group_settings(message.chat.id, 'auto_delete', True)
            asyncio.create_task(delete_messages_later(300, fuk, message))
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if AI_SPELL_CHECK == True and vj_search == True:
        ...
        try:
            ...
            grpid = await active_connection(str(reqstr1)) if reqstr1 else None
            if grpid:
                ...
                try:
                    if fileid == "None":
                        if btn == "[]":
                            ...
                            button = safe_literal_buttons(btn)
                            joelkb = await client.send_message(
                                group_id,
                                reply_text,
                                disable_web_page_preview=True,
